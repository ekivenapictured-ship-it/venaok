...
const SignaturePad: React.FC<SignaturePadProps> = ({ onSave, onClose, hideCloseButton, customSaveText, customClearText }) => {
        ...
        <div className="flex flex-col items-center w-full">
            ...
            <div className="flex w-full justify-between items-center mt-4">
                <button onClick={clearCanvas} disabled={!isSigned} className="button-secondary text-sm inline-flex items-center gap-2">
                    <Trash2Icon className="w-4 h-4" /> {customClearText || 'Ulangi'}
                </button>
                 <div className="space-x-2">
                    {!hideCloseButton && <button onClick={onClose} className="button-secondary text-sm">Batal</button>}
                    <button onClick={saveSignature} disabled={!isSigned} className="button-primary text-sm">{customSaveText || 'Simpan Tanda Tangan'}</button>
                </div>
            </div>
        </div>
    );
