...
import { Project, Client, Package, AddOn, PaymentStatus, AssignedTeamMember, TeamMember, ProjectStatusConfig, RevisionStatus } from '../types';
...
const Projects: React.FC = () => {
    const {
        ...
        getClients, getPackages, getTeamMembers, getProfile,
        isLoading, error
    } = useDatabase();
    ...
    const [profile, setProfile] = useState<any>({ projectStatusConfig: [], projectTypes: [] });
    ...
    const [printingItemForm, setPrintingItemForm] = useState<{ id: string; type: AddOn['type']; customName: string; details: string; cost: string }>({ id: '', type: 'Cetak Album', customName: '', details: '', cost: ''});
    ...
    const [confirmationModalState, setConfirmationModalState] = useState<{ project: Project; subStatus: any; isFollowUp: boolean; } | null>(null); // Use 'any' for subStatus for now
    ...
    const loadInitialData = async () => {
        const [projectsData, clientsData, packagesData, teamMembersData, profileData] = await Promise.all([
            getProjects(),
            getClients(),
            getPackages(),
            getTeamMembers(),
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (formMode === 'add') {
            ...
            projectData = await createProject(projectData);
            if (projectData) {
            ...
            projectData = { ...originalProject, ...formData, printingDetails: formData.printingDetails || [] };
            projectData = await updateProject(projectData.id, projectData);
            if (projectData) {
    ...
    const handleOpenBriefingModal = (project: Project) => {
        ...
        // Placeholder for other modal logic if needed
        setSelectedProject({...project, whatsappLink: whatsappLink}); // Pass whatsappLink to selectedProject for use in the modal
        setIsDetailModalOpen(true); // Assuming this will open a modal that includes briefing sharing. If not, adjust accordingly.
    };
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>, newStatus: string) => {
        ...
        if (projectToUpdate && projectToUpdate.status !== newStatus) {
            const updatedProject = { ...projectToUpdate, status: newStatus, progress: getProgressForStatus(newStatus, profile.projectStatusConfig), activeSubStatuses: [] };
            const success = await updateProject(projectId, updatedProject);
            if (success) {
                setProjects(prevProjects =>
                    prevProjects.map(p => p.id === projectId ? updatedProject : p)
                );
                setShowNotification(`Status "${projectToUpdate.projectName}" diubah ke "${newStatus}"`);
            }
        }
    ...
    const handleAddOrUpdatePrintingItem = () => {
        ...
        if (!details || !cost || (type === 'Custom' && !customName)) {
            setShowNotification('Harap lengkapi detail dan biaya item cetak.');
            return;
        }
        ...
        if (editingPrintingItemId) {
            ...
            const newItem: AddOn = { // Assuming AddOn type can be used for printing items
                id: `pi-${Date.now()}`,
                type,
                customName: type === 'Custom' ? customName : undefined,
                details,
                price: Number(cost), // Use price field
            };
    ...
    const handleEditPrintingItem = (item: AddOn) => {
    ...
    const handleOpenConfirmationModal = (project: Project, subStatus: any, isFollowUp: boolean) => {
    ...
    const getStatusClass = (status: string, config: ProjectStatusConfig[]) => {
        ...
        const color = statusConfig ? statusConfig.color : '#64748b'; // slate-500 default
    ...
    return (
        <div className="space-y-8">
            ...
            {showNotification && (
                <div className="p-3 bg-green-100 border border-green-400 text-green-700 rounded mb-4 text-center">
                    {showNotification}
                </div>
            )}
                                    ...
                                    <div className="space-y-2 mb-4">
                                        {(formData.printingDetails || []).map(item => (
                                            <div key={item.id} className="p-2 bg-brand-input rounded-lg flex justify-between items-center text-sm">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <p className="font-semibold text-brand-text-primary">{formatCurrency(item.price)}</p>
                                        ...
                                        {(formData.printingDetails || []).length > 0 &&
                                            <div className="text-right font-bold text-brand-text-light pt-2">
                                                Total Biaya Cetak: {formatCurrency((formData.printingDetails || []).reduce((sum, i) => sum + i.price, 0))}
                                            </div>
                                        }
                                    </div>
                                    ...
                                    {isAddingPrintingItem ? (
                                        <div className="p-4 bg-brand-bg rounded-xl border border-brand-border space-y-4">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                                                <div className="input-group !mt-0 sm:col-span-2">
                    ...
                    <div className="bg-brand-surface p-4 rounded-xl shadow-lg border border-brand-border flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="input-field !rounded-lg !border !bg-brand-bg p-2.5 w-full md:w-48">
                                ...
                                {profile.projectStatusConfig.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
                            </select>
            ...
            <Modal isOpen={!!(selectedProject && selectedProject.whatsappLink)} onClose={() => setSelectedProject(prev => prev ? {...prev, whatsappLink: undefined} : null)} title="Bagikan Briefing Proyek" size="2xl">
                {selectedProject && selectedProject.whatsappLink && (
                    <div className="space-y-4">
                        <textarea value={selectedProject.briefingText || ''} readOnly rows={15} className="input-field w-full text-sm"></textarea>
                        <div className="flex flex-col sm:flex-row justify-end items-center gap-3 pt-4 border-t border-brand-border">
                            {selectedProject.icsDataUri && <a href={selectedProject.icsDataUri} download={`${selectedProject.projectName}.ics`} className="button-secondary text-sm">Download .ICS</a>}
                            {selectedProject.googleCalendarLink && <a href={selectedProject.googleCalendarLink} target="_blank" rel="noopener noreferrer" className="button-secondary text-sm">Tambah ke Google Calendar</a>}
                            <a href={selectedProject.whatsappLink} target="_blank" rel="noopener noreferrer" className="button-primary inline-flex items-center gap-2">
                                ...
                                .map(p => (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-center mt-3 text-xs">
                                            ...
                                            <ConfirmationIcons project={p} />
                                            <span className={`px-2 py-1 rounded-full ${getPaymentStatusClass(p.paymentStatus)}`}>
                                                {p.paymentStatus}
                                            </span>
                                        </div>
                                    </div>
...
interface ProjectDetailModalProps {
    ...
    clients: Client[];
    profile: any; // Use specific type if available
    ...
    handleOpenConfirmationModal: (project: Project, subStatus: any, isFollowUp: boolean) => void; // Use specific type if available
    packages: Package[];
}
...
const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ selectedProject, setSelectedProject, teamMembers, clients, profile, showNotification, setProjects, onClose, handleOpenForm, handleProjectDelete, handleOpenBriefingModal, handleOpenConfirmationModal, packages }) => {
    ...
    const handleAddRevision = (e: React.FormEvent) => {
        ...
        const revisionToAdd: any = { // Use specific type if available
            id: `REV-${Date.now()}`,
    ...
    const handleShareRevisionLink = (revision: any) => { // Use specific type if available
    ...
    const handleToggleDigitalItem = (item: AddOn) => { // Use AddOn type
        if (!selectedProject) return;

        const currentCompleted = selectedProject.completedDigitalItems || [];
        const isCompleted = currentCompleted.includes(item.id); // Assuming item.id is used to track completion
        const newCompleted = isCompleted
            ? currentCompleted.filter((itemId: string) => itemId !== item.id)
                                        ...
                                        <ul className="space-y-2 text-sm">
                                            {(selectedProject.printingDetails || []).map(item => (
                                                <li key={item.id} className="flex justify-between items-center">
                                                    ...
                                                    <span className="text-brand-text-secondary font-medium">{formatCurrency(item.price)}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    ) : (
                                        ...
                                        return (
                                            <div className="space-y-2 text-sm">
                                                {pkg.digitalItems.map((item, index) => {
                                                    const isCompleted = selectedProject.completedDigitalItems?.includes(item.id);
                                                    return (
                                                        <label key={index} className="flex items-center gap-2 p-1.5 rounded-md hover:bg-brand-input cursor-pointer">
                                                            ...
                                                            />
                                                            <span className={`text-brand-text-primary ${isCompleted ? 'line-through text-brand-text-secondary' : ''}`}>{item.name}</span>
                                                        </label>
                                                    );
                                                })}
                                            </div>
                                    ...
                                    Object.entries(teamByRole).map(([role, members]) => (
                                        <div key={role}>
                                            ...
                                            <div className="mt-2 space-y-2">
                                                {members.map(member => (
                                                    <div key={member.id} className="p-3 bg-brand-bg rounded-lg flex flex-col sm:flex-row sm:justify-between sm:items-center">
                                                        <div>
...
const ConfirmationModal: React.FC<{
    project: Project;
    subStatus: any; // Use specific type if available
    isFollowUp: boolean;
    clients: Client[];
    teamMembers: TeamMember[];
    setProjects: React.Dispatch<React.SetStateAction<Project[]>>;
