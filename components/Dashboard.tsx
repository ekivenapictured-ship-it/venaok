...
import { useDatabase } from '../hooks/useDatabase'; // Assuming useDatabase hook is in ../hooks/useDatabase
...
const Dashboard: React.FC<DashboardProps> = ({ projects, clients, transactions, teamMembers, cards, pockets, handleNavigation, leads, teamProjectPayments, packages, assets, clientFeedback, contracts, currentUser, projectStatusConfig }) => {
  ...
  const { 
    getProjects: fetchProjects, getClients: fetchClients, getLeads: fetchLeads, getTransactions: fetchTransactions,
    isLoading, error 
  } = useDatabase();

  const [dashboardProjects, setDashboardProjects] = useState<Project[]>(projects);
  const [dashboardClients, setDashboardClients] = useState<Client[]>(clients);
  const [dashboardLeads, setDashboardLeads] = useState<Lead[]>(leads);
  const [dashboardTransactions, setDashboardTransactions] = useState<Transaction[]>(transactions);

  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        const [projectsData, clientsData, leadsData, transactionsData] = await Promise.all([
          fetchProjects(),
          fetchClients(),
          fetchLeads(),
          fetchTransactions()
        ...
        if (projectsData) setDashboardProjects(projectsData);
        if (clientsData) setDashboardClients(clientsData);
        if (leadsData) setDashboardLeads(leadsData);
        if (transactionsData) setDashboardTransactions(transactionsData);
      } catch (err) {
    ...
    loadDashboardData();
  }, [fetchProjects, fetchClients, fetchLeads, fetchTransactions]);
  ...
  const summary = useMemo(() => {
    return {
      ...
      activeProjects: dashboardProjects.filter(p => p.status !== 'Selesai' && p.status !== 'Dibatalkan').length,
      activeClients: dashboardClients.filter(c => c.status === ClientStatus.ACTIVE).length,
      totalFreelancers: teamMembers.length,
    };
  }, [cards, dashboardProjects, dashboardClients, teamMembers]);

  const activeProjects = useMemo(() => dashboardProjects.filter(p => p.status !== 'Selesai' && p.status !== 'Dibatalkan'), [dashboardProjects]);
  const activeClients = useMemo(() => dashboardClients.filter(c => c.status === ClientStatus.ACTIVE), [dashboardClients]);
  ...
  const getActiveClientsCount = () => {
    return dashboardClients.filter(c => c.status === 'ACTIVE').length;
  };
  ...
  const getNewLeadsCount = () => {
    ...
    return dashboardLeads.filter(l => new Date(l.date) > lastWeek).length;
  };
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      'Dalam Progress': 'bg-blue-100 text-blue-800',
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-96 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p><strong>Tipe:</strong> {project.projectType}</p>
            <p><strong>Tanggal:</strong> {new Date(project.date).toLocaleDateString('id-ID')}</p>
            <p><strong>Lokasi:</strong> {project.location}</p>
          </div>
          <div>
            <p><strong>Status:</strong> <span className={`px-2 py-1 rounded text-xs ${getStatusColor(project.status)}`}>{project.status}</span></p>
            <p><strong>Progress:</strong> {project.progress}%</p>
            <p><strong>Total Biaya:</strong> {formatCurrency(project.totalCost)}</p>
            <p><strong>Dibayar:</strong> {formatCurrency(project.amountPaid)}</p>
          </div>
        </div>
  ...
  return (
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="col-span-1 xl:col-span-8 widget-animate" style={{ animationDelay: '500ms' }}><IncomeChartWidget transactions={dashboardTransactions} /></div>
        <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '600ms' }}><UpcomingCalendarWidget projects={dashboardProjects} handleNavigation={handleNavigation} /></div>
        ...
        <div className="col-span-1 xl:col-span-5 widget-animate" style={{ animationDelay: '800ms' }}><RecentTransactionsWidget transactions={dashboardTransactions} /></div>

        <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '900ms' }}><ProjectStatusWidget projects={dashboardProjects} projectStatusConfig={projectStatusConfig} handleNavigation={handleNavigation} /></div>
        <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '1000ms' }}><LeadsSummaryWidget leads={dashboardLeads} handleNavigation={handleNavigation}/></div>
        ...
        <div className="col-span-1 xl:col-span-3 widget-animate cursor-pointer transition-transform duration-200 hover:scale-105" style={{ animationDelay: '1400ms' }} onClick={() => setActiveModal('contracts')}>
         ...
         <div className="col-span-1 xl:col-span-3 widget-animate" style={{ animationDelay: '1500ms' }}>
            <StatCard 
                ...
                value={
                    (() => {
                        const counts = dashboardProjects.reduce((acc, p) => { acc[p.packageName] = (acc[p.packageName] || 0) + 1; return acc; }, {} as Record<string, number>);
                            ...
                            {unpaidTeamPayments.length > 0 ? unpaidTeamPayments.map(p => (
                                <div key={p.id} className="p-3 bg-brand-bg rounded-lg flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-brand-text-secondary">Proyek: {dashboardProjects.find(proj => proj.id === p.projectId)?.projectName || 'N/A'}</p>
                                    </div>
