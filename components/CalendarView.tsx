...
export const CalendarView: React.FC<CalendarViewProps> = ({ projects, setProjects, teamMembers, profile, showNotification }) => {
    const { 
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedEvent) { // Editing
            ...
            const result = await updateProject(selectedEvent.id, updatedEventData);
            if (result) {
                setProjects(prev => prev.map(p => p.id === selectedEvent.id ? result : p));
                setSelectedEvent(result);
                setPanelMode('detail');
                showNotification(`Acara "${updatedEventData.projectName}" berhasil diperbarui.`);
            }
        } else { // Adding
            ...
            const result = await createProject(newEvent);
            if (result) {
                setProjects(prev => [...prev, result].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
                setIsPanelOpen(false);
                showNotification(`Acara "${newEvent.projectName}" berhasil ditambahkan.`);
            }
        }
    };
    ...
    const handleDeleteEvent = async () => {
        if (selectedEvent && window.confirm(`Yakin ingin menghapus acara "${selectedEvent.projectName}"?`)) {
            ...
            setIsPanelOpen(false);
            showNotification(`Acara "${selectedEvent.projectName}" berhasil dihapus.`);
        }
    };
