...
import { Transaction, TransactionType, Card, FinancialPocket, PocketType, Profile, Project, TeamMember, RewardLedgerEntry } from '../types';
...
const Finance: React.FC<FinanceProps> = ({ transactions, setTransactions, cards, setCards, pockets, setPockets, projects, profile, teamMembers, rewardLedgerEntries }) => {
    const { 
        getTransactions, createTransaction, updateTransaction, deleteTransaction,
        getCards, createCard, updateCard, deleteCard,
        getPockets, createPocket, updatePocket, deletePocket,
        isLoading, error 
    } = useDatabase();
    ...
    const [transactionForm, setTransactionForm] = useState<Omit<Transaction, 'id' | 'amount'> & { amount: string }>(emptyTransaction as any);

    const [isCardModalOpen, setIsCardModalOpen] = useState(false);
    const [editingCard, setEditingCard] = useState<Card | null>(null);
    const [cardForm, setCardForm] = useState<Omit<Card, 'id'> & { balance: string }>(emptyCard as any);
    ...
    const [pocketForm, setPocketForm] = useState<Omit<FinancialPocket, 'id'> & { amount: string, goalAmount?: string }>(emptyPocket as any);

    const resetTransactionForm = () => setTransactionForm(emptyTransaction as any);
    const resetCardForm = () => setCardForm(emptyCard as any);
    const resetPocketForm = () => setPocketForm(emptyPocket as any);
    ...
    const loadFinanceData = async () => {
        try {
            const [transactionsData, cardsData, pocketsData] = await Promise.all([
                getTransactions(),
                getCards(),
                getPockets()
            ]);
            ...
            showNotification("Gagal memuat data keuangan.");
        }
    };
        ...
        if (mode === 'add') {
            if (type === 'transaction') {
                setTransactionForm({
                    ...emptyTransaction,
                    date: new Date().toISOString().split('T')[0],
                    cardId: cards.find(c => c.id !== 'CARD_CASH')?.id || '',
                    sourceId: cards.find(c => c.id !== 'CARD_CASH')?.id ? `card-${cards.find(c => c.id !== 'CARD_CASH')?.id}` : ''
                });
            }
            if (type === 'pocket') setPocketForm(emptyPocket as any);
            if (type === 'card') setCardForm({ ...emptyCard, balance: '' });
            if (type === 'transfer') {
            ...
            if (type === 'transaction') {
                ...
                setTransactionForm({ ...data, amount: String(data.amount) });
            }
            if (type === 'card') {
                setEditingCard(data);
                setCardForm({ ...data, balance: String(data.balance) });
            }
            if (type === 'pocket') {
                setEditingPocket(data);
                setPocketForm({ ...data, amount: String(data.amount), goalAmount: data.goalAmount ? String(data.goalAmount) : '' });
            }
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (type === 'transaction') handleSubmitTransaction(e);
        else if (type === 'card') handleSubmitCard(e);
        else if (type === 'pocket') handleSubmitPocket(e);
        else if (type === 'transfer') handleSubmitTransfer(e);
        else if (type === 'topup-cash') handleSubmitTopupCash(e);
    };
    ...
    const handleSubmitCard = async (e: React.FormEvent) => {
        ...
        if (editingCard) {
            ...
            if (result) {
                ...
                showNotification(`Kartu ${cardForm.cardHolderName} berhasil diperbarui.`);
            }
        } else {
            const result = await createCard(cardData);
            if (result) {
                setCards(prev => [...prev, result]);
                showNotification(`Kartu ${cardForm.cardHolderName} berhasil ditambahkan.`);
            }
        }
    ...
    const handleSubmitPocket = async (e: React.FormEvent) => {
        e.preventDefault();

        const pocketData = {
            ...pocketForm,
            amount: Number(pocketForm.amount),
            goalAmount: pocketForm.goalAmount ? Number(pocketForm.goalAmount) : undefined
        };

        if (editingPocket) {
            const result = await updatePocket(editingPocket.id, pocketData);
            if (result) {
                setPockets(prev => prev.map(p => 
                    p.id === editingPocket.id ? result : p
                ));
                showNotification(`Kantong ${pocketForm.name} berhasil diperbarui.`);
            }
        } else {
            const result = await createPocket(pocketData);
            if (result) {
                setPockets(prev => [...prev, result]);
                showNotification(`Kantong ${pocketForm.name} berhasil ditambahkan.`);
            }
        }
    ...
    const handleSubmitTransfer = async (e: React.FormEvent) => {
        e.preventDefault();
        const { amount, fromCardId, toPocketId, type } = form; // form now holds transfer details
        ...
        const destinationPocket = pockets.find(p => p.id === toPocketId);

        if (!sourceCard || !destinationPocket) {
            alert("Kartu atau kantong tidak valid.");
            return;
        }

        if (type === 'withdraw') { // Withdraw from pocket to card
            if (destinationPocket.amount < transferAmount) {
                alert(`Saldo kantong ${destinationPocket.name} tidak mencukupi.`);
                return;
            }
            
            await updatePocket(destinationPocket.id, { ...destinationPocket, amount: destinationPocket.amount - transferAmount });
            await updateCard(sourceCard.id, { ...sourceCard, balance: sourceCard.balance + transferAmount });
            
            const transferTx: Transaction = {
                id: `TRN-TFR-${Date.now()}`,
                date: new Date().toISOString().split('T')[0],
                amount: transferAmount,
                description: `Tarik dari ${destinationPocket.name} ke ${sourceCard.bankName}`,
                type: TransactionType.INCOME, // Treated as income to the card
                category: 'Transfer Internal',
                method: 'Sistem',
                cardId: sourceCard.id,
                pocketId: destinationPocket.id,
            };
        ...
        } else { // Deposit from card to pocket
            if (sourceCard.balance < transferAmount) {
                alert(`Saldo kartu ${sourceCard.bankName} tidak mencukupi.`);
                return;
            }

            await updateCard(sourceCard.id, { ...sourceCard, balance: sourceCard.balance - transferAmount });
            await updatePocket(destinationPocket.id, { ...destinationPocket, amount: destinationPocket.amount + transferAmount, sourceCardId: sourceCard.id });

            const transferTx: Transaction = {
                id: `TRN-TFR-${Date.now()}`,
                date: new Date().toISOString().split('T')[0],
                amount: transferAmount,
                description: `Setor ke ${destinationPocket.name} dari ${sourceCard.bankName}`,
                type: TransactionType.EXPENSE, // Treated as expense from the card
                category: 'Transfer Internal',
                method: 'Sistem',
                cardId: sourceCard.id,
                pocketId: destinationPocket.id,
    ...
    const handleSubmitTopupCash = async (e: React.FormEvent) => {
        e.preventDefault();
        const { amount, fromCardId } = form;
        ...
        if (!sourceCard || !cashCard) {
            alert("Kartu sumber atau kartu tunai tidak valid.");
            return;
        }

        if (sourceCard.balance < topupAmount) {
            alert(`Saldo kartu ${sourceCard.bankName} tidak mencukupi.`);
            return;
        }

        await updateCard(sourceCard.id, { ...sourceCard, balance: sourceCard.balance - topupAmount });
        await updateCard(cashCard.id, { ...cashCard, balance: cashCard.balance + topupAmount });
        ...
        const topupTx: Transaction = {
            ...
            amount: topupAmount,
            type: TransactionType.EXPENSE, // Expense from the source card
            category: 'Transfer Internal',
            method: 'Sistem',
            cardId: sourceCard.id,
        };
        await createTransaction(topupTx);
    ...
    const handleCloseBudget = (budgetPocket: FinancialPocket, isAutomatic: boolean = false) => {
        ...
        if (budgetPocket.amount > 0) {
            ...
            // Create new pocket and update existing one
            Promise.all([
                createPocket(newSavedPocket),
                updatePocket(budgetPocket.id, { ...budgetPocket, amount: 0, name: currentMonthName }),
                createTransaction(closingTx)
            ]).then(([createdPocket, updatedBudgetPocket, createdTx]) => {
                setPockets(prev => [...prev, createdPocket].map(p =>
                    p.id === budgetPocket.id ? { ...updatedBudgetPocket, ...{ name: currentMonthName } } : p
                ));
                ...
                console.error("Failed to close budget:", err);
                showNotification("Gagal menutup anggaran.");
            });

        } else {
            // Just reset the pocket for the new month if amount is 0
            updatePocket(budgetPocket.id, { ...budgetPocket, name: currentMonthName }).then(updatedPocket => {
                setPockets(prev => prev.map(p => p.id === budgetPocket.id ? updatedPocket : p));
                if (isAutomatic) {
                    showNotification(`Anggaran bulanan telah diperbarui untuk ${now.toLocaleString('id-ID', { month: 'long', year: 'numeric' })}.`);
                }
            }).catch(err => {
                console.error("Failed to update budget name:", err);
                showNotification("Gagal memperbarui nama anggaran.");
            });
        }
    };
    ...
    useEffect(() => {
        ...
        autoCloseBudget();
    }, [pockets]); // Depend on pockets to re-evaluate if needed
    ...
    const handleReportFilterChange = (e: React.ChangeEvent<any>) => setReportFilters(prev => ({ ...prev, [e.target.name]: e.target.value }));
    const handleProfitReportFilterChange = (e: React.ChangeEvent<any>) => setProfitReportFilters(p => ({...p, [e.target.name]: e.target.value}));
                                            ...
                                            <tr key={t.id} className="hover:bg-brand-bg">
                                                ...
                                                <td className="p-3 text-center">
                                                    <div className="flex items-center justify-center space-x-1">
                                                        ...
                                                        <button onClick={() => handleDeleteTransaction(t.id)} className="p-2 text-brand-text-secondary hover:bg-brand-input rounded-full"><Trash2Icon className="w-4 h-4"/></button>
                                                    </div>
                                                </td>
                                            </tr>
                                          );
                            ...
                            return (
                                <div key={card.id}>
                                    {card.id === 'CARD_CASH' 
                                        ...
                                        : <CardWidget card={card} onEdit={() => handleOpenModal('card', 'edit', card)} onDelete={() => handleDeleteCard(card.id)} onClick={() => setHistoryModalState({type: 'card', item: card})} />
                                    }
                    ...
                    <div className="space-y-6 printable-area widget-animate">
                        <div className="bg-brand-surface p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center non-printable border border-brand-border">
                            <h4 className="text-md font-semibold text-gradient whitespace-nowrap">Filter Laporan:</h4>
                             <select name="client" value={reportFilters.client} onChange={handleReportFilterChange} className="input-field !rounded-lg !border p-2.5 w-full md:w-auto"><option value="all">Semua Klien</option>{reportClientOptions.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                            <input type="date" name="dateFrom" value={reportFilters.dateFrom} onChange={handleReportFilterChange} className="input-field !rounded-lg !border p-2.5 w-full md:w-auto"/>
                            <input type="date" name="dateTo" value={reportFilters.dateTo} onChange={handleReportFilterChange} className="input-field !rounded-lg !border p-2.5 w-full md:w-auto"/>
                ...
                return (
                    <div className="space-y-6 printable-area widget-animate">
                        <div className="bg-brand-surface p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center non-printable border border-brand-border">
                            ...
                            <select name="year" value={profitReportFilters.year} onChange={handleProfitReportFilterChange} className="input-field !rounded-lg !border p-2.5 w-full md:w-auto">
                                {yearOptions.map(y => <option key={y} value={y}>{y}</option>)}
                            </select>
                            <select name="month" value={profitReportFilters.month} onChange={handleProfitReportFilterChange} className="input-field !rounded-lg !border p-2.5 w-full md:w-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {modalState.type === 'transaction' && <>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="input-group"><select id="type" name="type" value={transactionForm.type} onChange={(e) => setTransactionForm(p => ({...p, type: e.target.value as TransactionType}))} className="input-field"><option value={TransactionType.EXPENSE}>Pengeluaran</option><option value={TransactionType.INCOME}>Pemasukan</option></select><label htmlFor="type" className="input-label">Jenis</label></div>
                            <div className="input-group"><input type="date" id="date" name="date" value={transactionForm.date} onChange={(e) => setTransactionForm(p => ({...p, date: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="date" className="input-label">Tanggal</label></div>
                        </div>
                        <div className="input-group"><input type="text" id="description" name="description" value={transactionForm.description} onChange={(e) => setTransactionForm(p => ({...p, description: e.target.value}))} className="input-field" placeholder=" " required/><label htmlFor="description" className="input-label">Deskripsi</label></div>
                        <div className="input-group"><input type="number" id="amount" name="amount" value={transactionForm.amount} onChange={(e) => setTransactionForm(p => ({...p, amount: e.target.value}))} className="input-field" placeholder=" " required/><label htmlFor="amount" className="input-label">Jumlah (IDR)</label></div>
                        <div className="input-group"><select id="category" name="category" value={transactionForm.category} onChange={(e) => setTransactionForm(p => ({...p, category: e.target.value}))} className="input-field" required><option value="">Pilih Kategori...</option>{(transactionForm.type === TransactionType.INCOME ? profile.incomeCategories : profile.expenseCategories).map(cat => (<option key={cat} value={cat}>{cat}</option>))}</select><label htmlFor="category" className="input-label">Kategori</label></div>
                        {transactionForm.type === TransactionType.INCOME ? (
                            <div className="input-group"><select id="cardId" name="cardId" value={transactionForm.cardId || ''} onChange={(e) => setTransactionForm(p => ({...p, cardId: e.target.value}))} className="input-field" required><option value="">Pilih Tujuan...</option>{cards.map(c => (<option key={c.id} value={c.id}>{c.bankName} {c.lastFourDigits !== 'CASH' ? `**** ${c.lastFourDigits}` : ''} (Saldo: {formatCurrency(c.balance)})</option>))}</select><label htmlFor="cardId" className="input-label">Setor Ke Kartu/Tunai</label></div>
                        ) : (
                             <div className="input-group"><select id="sourceId" name="sourceId" value={transactionForm.sourceId} onChange={(e) => setTransactionForm(p => ({...p, sourceId: e.target.value}))} className="input-field" required><option value="">Pilih Sumber...</option>{cards.map(c => (<option key={c.id} value={`card-${c.id}`}>{c.bankName} {c.lastFourDigits !== 'CASH' ? `**** ${c.lastFourDigits}` : ''} (Saldo: {formatCurrency(c.balance)})</option>))}{(pockets.filter(p=>p.type === PocketType.EXPENSE).map(p => (<option key={p.id} value={`pocket-${p.id}`}>{p.name} (Sisa: {formatCurrency(p.amount)})</option>)))}</select><label htmlFor="sourceId" className="input-label">Sumber Dana</label></div>
                        )}
                    </>}
                    {modalState.type === 'card' && <>
                         <div className="input-group"><input type="text" id="bankName" name="bankName" value={cardForm.bankName} onChange={(e) => setCardForm(p => ({...p, bankName: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="bankName" className="input-label">Nama Bank</label></div>
                        <div className="input-group"><input type="text" id="cardHolderName" name="cardHolderName" value={cardForm.cardHolderName} onChange={(e) => setCardForm(p => ({...p, cardHolderName: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="cardHolderName" className="input-label">Nama Pemegang Kartu</label></div>
                        {modalState.mode === 'add' && (
                            <div className="input-group">
                                <input type="number" id="balance" name="balance" value={cardForm.balance} onChange={(e) => setCardForm(p => ({...p, balance: e.target.value}))} className="input-field" placeholder=" " />
                                <label htmlFor="balance" className="input-label">Saldo Awal (IDR)</label>
                            </div>
                        )}
                        {form.id !== 'CARD_CASH' && (
                            <div className="grid grid-cols-2 gap-4">
                                <div className="input-group"><input type="text" id="lastFourDigits" name="lastFourDigits" value={cardForm.lastFourDigits} onChange={(e) => setCardForm(p => ({...p, lastFourDigits: e.target.value}))} className="input-field" placeholder=" " maxLength={4}/><label htmlFor="lastFourDigits" className="input-label">4 Digit Terakhir</label></div>
                                <div className="input-group"><input type="text" id="expiryDate" name="expiryDate" value={cardForm.expiryDate} onChange={(e) => setCardForm(p => ({...p, expiryDate: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="expiryDate" className="input-label">Kadaluwarsa (MM/YY)</label></div>
                            </div>
                        )}
                    </>}
                    {modalState.type === 'pocket' && <>
                        <div className="input-group"><input type="text" id="name" name="name" value={pocketForm.name} onChange={(e) => setPocketForm(p => ({...p, name: e.target.value}))} className="input-field" placeholder=" " required /><label htmlFor="name" className="input-label">Nama Kantong</label></div>
                        <div className="input-group"><textarea id="description" name="description" value={pocketForm.description} onChange={(e) => setPocketForm(p => ({...p, description: e.target.value}))} className="input-field" placeholder=" " rows={2}/><label htmlFor="description" className="input-label">Deskripsi</label></div>
                        <div className="input-group"><select id="sourceCardId" name="sourceCardId" value={pocketForm.sourceCardId} onChange={(e) => setPocketForm(p => ({...p, sourceCardId: e.target.value}))} className="input-field"><option value="">Pilih Kartu...</option>{cards.map(c => <option key={c.id} value={c.id}>{c.bankName} {c.id !== 'CARD_CASH' && `**** ${c.lastFourDigits}`}</option>)}</select><label htmlFor="sourceCardId" className="input-label">Sumber Dana (Kartu)</label></div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="input-group"><select id="icon" name="icon" value={pocketForm.icon} onChange={(e) => setPocketForm(p => ({...p, icon: e.target.value as FinancialPocket['icon']}))} className="input-field">{Object.keys(pocketIcons).map(i => <option key={i} value={i}>{i}</option>)}</select><label htmlFor="icon" className="input-label">Ikon</label></div>
                            <div className="input-group">
                                <select name="type" id="type" value={pocketForm.type} onChange={(e) => setPocketForm(p => ({...p, type: e.target.value as PocketType}))} className="input-field">
                        ...
                        {(pocketForm.type === PocketType.SAVING || pocketForm.type === PocketType.EXPENSE) &&
                            <div className="input-group">
                                <input type="number" id="goalAmount" name="goalAmount" value={pocketForm.goalAmount || ''} onChange={(e) => setPocketForm(p => ({...p, goalAmount: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="goalAmount" className="input-label">Target Jumlah (Opsional)</label>
                            </div>
                        }
                        {pocketForm.type === PocketType.LOCKED &&
                            <div className="input-group">
                                <input type="date" id="lockEndDate" name="lockEndDate" value={pocketForm.lockEndDate || ''} onChange={(e) => setPocketForm(p => ({...p, lockEndDate: e.target.value}))} className="input-field" placeholder=" "/><label htmlFor="lockEndDate" className="input-label">Tgl. Kunci Berakhir (Opsional)</label>
                            </div>
                        }
                    </>}
