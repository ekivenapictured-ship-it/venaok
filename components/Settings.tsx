...
const Settings: React.FC<SettingsProps> = ({ 
    ...
    const handleUserFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setUserFormError('Terjadi kesalahan. Silakan coba lagi.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const updateProfileCategory = async (field: keyof Profile, newValue: any) => {
        setIsLoading(true);
        try {
            const updatedProfile = { ...profile, [field]: newValue };
            setProfile(updatedProfile);
            await database.updateProfile({ [field]: newValue });
        } catch (error) {
    ...
    const handleAddOrUpdateIncomeCategory = async () => {
        ...
        if (editingIncomeCategory) {
            ...
            const updatedCategories = categories.map(c => c === editingIncomeCategory ? newCategory : c).sort();
            await updateProfileCategory('incomeCategories', updatedCategories);
    ...
    const handleDeleteIncomeCategory = async (category: string) => {
        ...
        if (window.confirm(`Yakin ingin menghapus kategori "${category}"?`)) {
            const updatedCategories = (profile.incomeCategories || []).filter(c => c !== category);
            await updateProfileCategory('incomeCategories', updatedCategories);
        }
    };
    ...
    const handleAddOrUpdateExpenseCategory = async () => {
        ...
        if (editingExpenseCategory) {
            ...
            const updatedCategories = categories.map(c => c === editingExpenseCategory ? newCategory : c).sort();
            await updateProfileCategory('expenseCategories', updatedCategories);
    ...
    const handleDeleteExpenseCategory = async (category: string) => {
        ...
        if (window.confirm(`Yakin ingin menghapus kategori "${category}"?`)) {
            const updatedCategories = (profile.expenseCategories || []).filter(c => c !== category);
            await updateProfileCategory('expenseCategories', updatedCategories);
        }
    };
    ...
    const handleAddOrUpdateProjectType = async () => {
        ...
        if (editingProjectType) {
            ...
            const updatedTypes = types.map(t => t === editingProjectType ? newType : t).sort();
            await updateProfileCategory('projectTypes', updatedTypes);
            setEditingProjectType(null);
        } else {
            if (types.includes(newType)) {
                alert('Jenis proyek ini sudah ada.'); return;
            }
            const updatedTypes = [...types, newType].sort();
            await updateProfileCategory('projectTypes', updatedTypes);
        }
        setProjectTypeInput('');
    };
    ...
    const handleDeleteProjectType = async (type: string) => {
        ...
        if (window.confirm(`Yakin ingin menghapus jenis proyek "${type}"?`)) {
            const updatedTypes = (profile.projectTypes || []).filter(t => t !== type);
            await updateProfileCategory('projectTypes', updatedTypes);
        }
    };
    ...
    const handleAddOrUpdateEventType = async () => {
        ...
        if (editingEventType) {
            ...
            const updatedTypes = types.map(t => t === editingEventType ? newType : t).sort();
            await updateProfileCategory('eventTypes', updatedTypes);
    ...
    const handleDeleteEventType = async (type: string) => {
        ...
        if (window.confirm(`Yakin ingin menghapus jenis acara "${type}"?`)) {
            const updatedTypes = (profile.eventTypes || []).filter(t => t !== type);
            await updateProfileCategory('eventTypes', updatedTypes);
        }
    };

    const handleAddOrUpdateSopCategory = async () => {
        if (!sopCategoryInput.trim()) return;
        const newCat = sopCategoryInput.trim();
        const cats = profile.sopCategories || [];
        
        if (editingSopCategory) {
            if (newCat !== editingSopCategory && cats.includes(newCat)) { 
                alert('Kategori ini sudah ada.'); return; 
            }
            const updatedCats = cats.map(c => c === editingSopCategory ? newCat : c).sort();
            await updateProfileCategory('sopCategories', updatedCats);
            setEditingSopCategory(null);
        } else {
            if (cats.includes(newCat)) { 
                alert('Kategori ini sudah ada.'); return; 
            }
            const updatedCats = [...cats, newCat].sort();
            await updateProfileCategory('sopCategories', updatedCats);
        }
        setSopCategoryInput('');
    };
    
    const handleEditSopCategory = (cat: string) => { 
        setEditingSopCategory(cat); 
        setSopCategoryInput(cat); 
    };
    
    const handleDeleteSopCategory = async (cat: string) => {
        if (window.confirm(`Yakin ingin menghapus kategori SOP "${cat}"?`)) {
            const updatedCats = (profile.sopCategories || []).filter(c => c !== cat);
            await updateProfileCategory('sopCategories', updatedCats);
        }
    };
                 ...
                 return (
                    <form onSubmit={handleSubmit}>
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4 max-w-2xl mx-auto">
                                <div className="flex justify-between items-center"><label htmlFor="notif-newProject">Proyek Baru Dibuat</label><ToggleSwitch id="notif-newProject" enabled={profile.notificationSettings?.newProject || false} onChange={() => handleNotificationChange('newProject')} /></div>
                                <div className="flex justify-between items-center"><label htmlFor="notif-paymentConfirmation">Konfirmasi Pembayaran Diterima</label><ToggleSwitch id="notif-paymentConfirmation" enabled={profile.notificationSettings?.paymentConfirmation || false} onChange={() => handleNotificationChange('paymentConfirmation')} /></div>
                                <div className="flex justify-between items-center"><label htmlFor="notif-deadlineReminder">Pengingat Deadline Proyek</label><ToggleSwitch id="notif-deadlineReminder" enabled={profile.notificationSettings?.deadlineReminder || false} onChange={() => handleNotificationChange('deadlineReminder')} /></div>
                            </div>

                            <h3 className="text-lg font-semibold text-brand-text-light border-b border-gray-700/50 pb-3 mt-8">Keamanan</h3>
                            <div className="space-y-4 max-w-2xl mx-auto">
                                <div className="flex justify-between items-center"><label htmlFor="security-2fa">Autentikasi Dua Faktor (2FA)</label><ToggleSwitch id="security-2fa" enabled={profile.securitySettings?.twoFactorEnabled || false} onChange={() => setProfile(p => ({ ...p, securitySettings: { ...p.securitySettings, twoFactorEnabled: !p.securitySettings?.twoFactorEnabled } }))} /></div>
                            </div>

                            <div className="text-right mt-8 pt-6 border-t border-gray-700/50">
                                <button type="submit" disabled={isLoading} className="button-primary relative">
                ...
                return (
                    <div>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => handleOpenUserModal('add')} disabled={isLoading} className="button-primary inline-flex items-center gap-2"><PlusIcon className="w-5 h-5"/>Tambah Pengguna</button>
                        </div>
                        <div className="space-y-3">
                            {users.map(user => (
                                <div key={user.id} className="p-3 bg-brand-bg rounded-lg flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => handleOpenUserModal('edit', user)} disabled={isLoading} className="p-2 text-brand-text-secondary hover:bg-brand-input rounded-full" title="Edit"><PencilIcon className="w-5 h-5"/></button>
                                        <button onClick={() => handleDeleteUser(user.id)} disabled={isLoading} className="p-2 text-brand-text-secondary hover:bg-brand-input rounded-full" title="Hapus"><Trash2Icon className="w-5 h-5"/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                return (
                    <ProjectStatusManager
                        ...
                        onConfigChange={async (newConfig) => {
                            ...
                            await updateProfileCategory('projectStatusConfig', newConfig);
                        }}
                        projects={projects}
                    />
                );
            ...
            <Modal isOpen={isUserModalOpen} onClose={handleCloseUserModal} title={userModalMode === 'add' ? 'Tambah Pengguna Baru' : 'Edit Pengguna'}>
                <form onSubmit={handleUserFormSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-brand-border">
                        ...
                        <button type="submit" disabled={isLoading} className="button-primary">{isLoading ? 'Menyimpan...' : (userModalMode === 'add' ? 'Simpan Pengguna' : 'Update Pengguna')}</button>
                    </div>
                </form>
            </Modal>
        </div>
